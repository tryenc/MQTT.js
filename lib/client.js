...
MqttClient.prototype.end = function () {
  debug('client end - close connection')
  var that = this
  ...
  function finish () {
    ...
    // ok for websockets
    debug('calling _cleanUp')
    that._cleanUp(force, setImmediate.bind(null, closeStores), opts)
  }

  if (this.disconnecting) {
    // debug('client already disconnecting. returning with callback')
    // if (cb) {
    //   cb.apply(null, arguments)
    // }
    return this
  ...
  if (!force && Object.keys(this.outgoing).length > 0) {
    ...
    debug('calling finish in 10ms once outgoing is empty')
    this.once('outgoingEmpty', setTimeout.bind(null, finish, 10))
  } else {
    debug('immediately calling finish')
    finish()
  }
...
MqttClient.prototype._cleanUp = function (forced, done) {
  ...
  if (forced) {
    ...
    debug('_cleanUp: sending disconnect packet')
    this._sendPacket(
      packet,
      setImmediate.bind(
        null,
